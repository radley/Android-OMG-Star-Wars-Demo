...
public class SearchActivity extends AppCompatActivity implements BaseCategoryFragment.OnHeadlineSelectedListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(intent.hasExtra(SearchExtras.CATEGORY)) {
            mViewModel.setCurrentCategory(intent.getStringExtra(SearchExtras.CATEGORY));
        }
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        mSearchView.setQueryHint("Search " + mViewModel.getCurrentCategory() + "...");
    ...
    protected void initLayout() {
        ...
        ArrayAdapter<String> adapter = new ArrayAdapter<String>(this, R.layout.spinner_item, mViewModel.getCategoryTitles());

        adapter.setDropDownViewResource(R.layout.spinner_dropdown_item);
        mSpinner = (Spinner) findViewById(R.id.spinner);
        mSpinner.setAdapter(adapter);

        mSpinner.setSelection(mViewModel.getCurrentCategoryPosition());
        mSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {

            @Override
            public void onItemSelected(AdapterView<?> parentView, View selectedItemView, int position, long id) {

                if(!mViewModel.getCurrentCategory().equals(mViewModel.getCurrentCategory(position))) {

                    mViewModel.setCurrentCategory(mViewModel.getCurrentCategory(position));
                    mSearchView.clearFocus();
                    mResultsText.setText("");
